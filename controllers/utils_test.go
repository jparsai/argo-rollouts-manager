...
var _ = Describe("checkForExistingRolloutManager tests", func() {
	...
	BeforeEach(func() {
		s := scheme.Scheme
		Expect(rolloutsmanagerv1alpha1.AddToScheme(s)).ToNot(HaveOccurred())
	...
	When("A single cluster scoped rolloutsManager is created.", func() {
		It("it should not return any error, as it is a valid use case.", func() {

			By("Create only one RolloutManager.")
			Expect(k8sClient.Create(ctx, &rolloutsManager)).ToNot(HaveOccurred())

			By("Verify there is no error returned.")
			Expect(checkForExistingRolloutManager(ctx, k8sClient, &rolloutsManager, log)).ToNot(HaveOccurred())
		})
	})

	When("Multiple rolloutsManager are created and all are namespace scoped RMs.", func() {
		It("it should not return error for any of them, because only one cluster scoped RM or all namespace scoped RMs are allowed.", func() {
			...
			Expect(k8sClient.Create(ctx, &rolloutsManager2)).NotTo(HaveOccurred())

			By("2nd RM : Verify there is no error returned, as all namespace scoped RMs are created.")
			Expect(checkForExistingRolloutManager(ctx, k8sClient, &rolloutsManager2, log)).ToNot(HaveOccurred())

			By("1st RM : Recheck and it should still work, as all namespace scoped RMs are created.")
			Expect(k8sClient.Get(ctx, types.NamespacedName{Name: rolloutsManager.Name, Namespace: rolloutsManager.Namespace}, &rolloutsManager)).NotTo(HaveOccurred())
			Expect(checkForExistingRolloutManager(ctx, k8sClient, &rolloutsManager, log)).ToNot(HaveOccurred())
		})
	})

	When("Multiple rolloutsManager are created, including cluster scoped RMs.", func() {
		It("it should return error for all of them, because only one cluster scoped RM or all namespace scoped RMs are allowed.", func() {
			...
			Expect(rolloutsManager2.Status.Phase).To(Equal(rolloutsmanagerv1alpha1.PhasePending))
			Expect(rolloutsManager2.Status.RolloutController).To(Equal(rolloutsmanagerv1alpha1.PhasePending))

			By("1st RM : Recheck 1st RM and it should also have error now. since multiple RMs are created.")
			Expect(k8sClient.Get(ctx, types.NamespacedName{Name: rolloutsManager2.Name, Namespace: rolloutsManager2.Namespace}, &rolloutsManager2)).NotTo(HaveOccurred())

			err = checkForExistingRolloutManager(ctx, k8sClient, &rolloutsManager, log)
			Expect(err).To(HaveOccurred())
			Expect(doMultipleRolloutManagersExist(err)).To(BeTrue())
			Expect(rolloutsManager.Status.Phase).To(Equal(rolloutsmanagerv1alpha1.PhasePending))
			Expect(rolloutsManager.Status.RolloutController).To(Equal(rolloutsmanagerv1alpha1.PhasePending))
		})
	})
})
